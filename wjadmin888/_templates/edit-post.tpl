				...
				<div id="company_info" class="left span1 block last">
					...
					<div class="block_inner">
						...
						<div class="group_submit">
							...
							<button type="button" id="save"><span>{if $job.id == 0}Post job{else}Save changes{/if}</span></button>
							<input type="hidden" name="show_preview" id="show_preview" value="false" />
							<input type="hidden" name="job_id" value="{$job.id}" />
							<input type="hidden" id="base_url_admin" name="base_url_admin" value="{$BASE_URL_ADMIN}" />
						</div>
					</div>
				</div>
			</form>
