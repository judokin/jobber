...
{foreach item=job from=$jobs name=tmp}
	<div id="item{$job.id}" class="row {cycle values='odd,even'}">
		...
		<div>viewed {$job.views_count} times{if $statisticalData[$job.id]}, 
			{$statisticalData[$job.id].numberOfApplications} applicants, 
			last application on {$statisticalData[$job.id].lastApplicationOn}{/if}
			{if $spamReportStatisticalData[$job.id]},
			<span class="spam"> reported as spam {$spamReportStatisticalData[$job.id].numberOfSpamReports} times, 
			last time on {$spamReportStatisticalData[$job.id].lastSpamReportOn}</span>
			{/if}
			&nbsp;&nbsp;<a href="{$BASE_URL_ADMIN}jobhits/{$job.id}">增加一个view</a>
			&nbsp;&nbsp;<a href="{$BASE_URL_ADMIN}jobapp/{$job.id}">增加一个申请</a>
		</div>
	</div>
{/foreach}
</div>
