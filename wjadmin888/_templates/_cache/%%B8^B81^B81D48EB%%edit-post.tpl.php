<?php /* Smarty version 2.6.26, created on 2013-04-24 10:52:36
...
<?php endif; ?>" />
						...
						<div class="group_submit">
							...
							<button type="button" id="save"><span><?php if ($this->_tpl_vars['job']['id'] == 0): ?>Post job<?php else: ?>Save changes<?php endif; ?></span></button>
...
" />
							<input type="hidden" id="base_url_admin" name="base_url_admin" value="<?php echo $this->_tpl_vars['BASE_URL_ADMIN']; ?>
" />
						</div>
					</div>
				</div>
		...
		<script type="text/javascript">
			$(document).ready(function()
			{
				...
				{   
				    if($.trim($("#job_id").val())===\'\'){
				    var url = $("#base_url_admin").val()+\'checkjob?title=\'+$(\'#title\').val()+\'&desc=\'+$(\'#description\').val();
					//alert(url);
				    $.get(url,function(e){
					   if(e===\'1\'){
					    alert("已经有完全一样的职位了"); 
