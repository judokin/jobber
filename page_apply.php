<?php
    ...
    if (
	    $_FILES['apply_cv']['size']!=0&&
	    (
	    ($_FILES['apply_cv']['type'] != 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')&&
   	    ($_FILES['apply_cv']['type'] != "text/plain")&&
		($_FILES['apply_cv']['type'] != "application/pdf") &&
		($_FILES['apply_cv']['type'] != "application/msword")&&
		($_FILES['apply_cv']['type'] != "application/rtf")
		)
		){
	...
	if (empty($errors))
	{
		...
		$filename = time() . '_.' . end(explode('.',$_FILES['apply_cv']['name']));
