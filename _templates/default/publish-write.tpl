			...
			<form id="publish_form" method="post" action="{$BASE_URL}post/">
				<fieldset>
					...
					<table border="0" cellspacing="2" cellpadding="2">
						<tr>
							<td colspan="{if $ismobile}1{else}2{/if}">
						...
						<tr>
							...
							{/if}
							<td>
								<input {if $errors.title}class="error"{/if} type="text" name="title" id="title" tabindex="2" size="30" value="{if $job.company}{$job.title|escape}{else}{$smarty.post.title|escape}{/if}" />
						...
						<tr>
							...
							{/if}
							<td><textarea {if $errors.description}class="error"{/if} tabindex="4" name="description" id="description" cols="{if $ismobile}30{else}80{/if}" rows="15">{if $job.company}{$job.description}{else}{$smarty.post.description}{/if}</textarea>
