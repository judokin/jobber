						...
						<form id="frm-apply-online" method="post" enctype="multipart/form-data" action="{$BASE_URL}apply-online/">
							<table>
								...
								<tr>
									...
									{/if}
								    <td>
										<textarea {if $smarty.session.apply_errors.apply_msg}class="error"{/if} name="apply_msg" id="apply_msg" cols="{if $ismobile}30{else}60{/if}" rows="15">{$smarty.session.apply_fields.apply_msg}</textarea>
