				...
				{if $job.apply_online == 1 && $CURRENT_PAGE != 'verify'}
					...
					<div id="apply-online" {if $smarty.session.apply_successful AND $smarty.session.apply_successful eq -1}style="display: block;"{else}style="display: none;"{/if}>
						<form id="frm-apply-online" method="post" enctype="multipart/form-data" action="{$BASE_URL}apply-online/">
							<label for="apply_name">{$translations.apply.name_label}:</label><br />
							<input {if $smarty.session.apply_errors.apply_name}class="error"{/if} type="text" name="apply_name" id="apply_name" maxlength="50" size="30" value="{$smarty.session.apply_fields.apply_name}" placeholder="Your name" /><br />
							...
							<label for="apply_email">{$translations.apply.email_label}:</label><br />
							<input {if $smarty.session.apply_errors.apply_email}class="error"{/if} type="text" name="apply_email" id="apply_email" maxlength="50" size="30" value="{$smarty.session.apply_fields.apply_email}"  placeholder="name@domain.com" /><br />
							...
							<label for="apply_msg">{$translations.apply.message_label}:</label><br />
							<textarea {if $smarty.session.apply_errors.apply_msg}class="error"{/if} name="apply_msg" id="apply_msg" cols="60" rows="15" placeholder="Your message" >{$smarty.session.apply_fields.apply_msg}</textarea><br />
							...
							<label for="apply_cv">{$translations.apply.cv_label}:</label><br />
							...
							<p class="suggestion">{$translations.apply.cv_info}</p>
								...
								{/if}
							<input type="submit" name="submit" id="submit" value="{$translations.apply.submit}" />
							<p>{$translations.apply.or} <a href="#" onclick="$('#apply-online').toggle(); return false;">{$translations.apply.cancel}</a></p>
